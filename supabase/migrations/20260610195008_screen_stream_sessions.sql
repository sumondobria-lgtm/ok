...
CREATE POLICY "select_own_screen_sessions" ON screen_stream_sessions FOR SELECT
  TO authenticated USING (
    device_id IN (SELECT id FROM devices WHERE user_id = auth.uid())
  );

CREATE POLICY "insert_own_screen_sessions" ON screen_stream_sessions FOR INSERT
  TO authenticated WITH CHECK (
    device_id IN (SELECT id FROM devices WHERE user_id = auth.uid())
  );

CREATE POLICY "update_own_screen_sessions" ON screen_stream_sessions FOR UPDATE
  TO authenticated USING (
    device_id IN (SELECT id FROM devices WHERE user_id = auth.uid())
  );
